...
# Define file paths and projection settings
  ...
  data="data/occ_epIGbackbone.Rdata"# Input data file (EpIG backbone)
  ...
  poly <- sf::st_read(here("wgsrpd-master", wgsrpd), quiet = TRUE) %>%
    sf::st_transform(crs = projection) %>%
    as("Spatial")  # rgdal retired; sf equivalent
  ...
  for (i in 1:length(poly)) {
    cat(i,"\r")
    a <- poly[i,] %>% sf::st_as_sf() %>% sf::st_buffer(dist = 0.5) %>% as("Spatial")  # buffer via sf (rgeos retired)
...
# Repeat the process for 0.5-degree, and 0.25-degree
 ...
 # Map species occurrences to 0.5-degree grid
  ...
  ### FIFTH DEGREE — skipped for this run (outputs already exist in interim/)
  # Uncomment this block only if you need to regenerate the 15 arc-min grid
  # g <- raster::raster(nrows=180*5,ncols=360*5,xmn=-180,xmx=180,ymn=-90,ymx=90,vals=1)
  # ... (see backup 02_Datagrid_creation_for_each_resolution_ORIGINAL_GBIF.Rmd)

  # Combine grid resolutions — 30 arc-min only for this run
