...
```{r}
     ...
     # Define input data paths and parameters
      data ="data/occ_epIGbackbone.Rdata"# Input occurrence data

     resolution = 60# Grid resolution in arc-minutes
     
     data_comp="output/completeness/Main60exp/Estimators.RData" # Completeness/Slope data
...
# Load and process WGSRPD shapefile for regions of interest
  ...
  # Create buffered polygons for each region
     ...
     some %>% dplyr::select(decimalLongitude,decimalLatitude, scientificName, family) %>% 
       sf::st_as_sf(x = ., coords = c("decimalLongitude", "decimalLatitude"),
                    crs = "+proj=laea +lat_0=-10 +lon_0=-71 +x_0=0 +y_0=0 +a=6370997 +b=6370997 +units=m +no_defs") %>%
     ...
     richness %<>% 
  dplyr::select(-scientificName.x, -family.x) %>% 
  rename(scientificName = scientificName.y, family = family.y)
     ...
     datos %>% dplyr::select(Longitude,Latitude) %>% 
       ...
       rename_with(.,~all_of(c("name","CellID"))) %>%
       dplyr::select(CellID) %>% bind_cols(.,datos) -> datos
...
# Estimate metrics per grid-cell
     ...
     data ="data/occ_epIGbackbone.Rdata" #change!
