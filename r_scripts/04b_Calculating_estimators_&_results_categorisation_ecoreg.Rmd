...
Here the code: [ATM this script is a copy of the 4th and it was modified only until line 97, the calculation for polygons, in case the reviewers ask for it](it will remain in git ignore untill fully modified and in case is needed).

1. Calculates the estimators (at resolutions 30 and 15 minute of arc);
2. Sets the results in categories according to Lobo et al. (2018);
3. Plots the results;
4. Saves objects of results and plots.

Bibliographic references:

- Lobo, J.M., Hortal, J., Yela, J.L., ..., & Guisande, C. (2018) KnowBR: An application to map the geographical variation of survey effort and identify well-surveyed areas from biodiversity databases. Ecological Indicators, 91, 241–248. doi:10.1016/j.ecolind.2018.03.077
...
```{r}
  ...
  #Load neotropics shapefile
# Shapefile into SpatialPolygons
library('maptools')
WWF_Poly <- readShapePoly("F:/Academic/b_Projects/CONTINENT/Manuscripts/LatitudinalDivesityEpis/Analyses/R_Script/DONE/data/Neotropics/neotropics-wwf/neotropics-wwf.shp",
                          proj4string =
                            CRS("+proj=laea +lat_0=-10 +lon_0=-71 +x_0=0 +y_0=0 +a=6370997 +b=6370997 +units=m +no_defs"))
#
# changing to SpatialPolygons/SpatialVector
WWF_Poly_spa_polyg <- SpatialPolygons(WWF_Poly@polygons)
WWF_Poly_spa_polyg_names <- names(WWF_Poly_spa_polyg)  
...
cutoffSlope = 1, 
extent=TRUE, #minLon, maxLon, minLat, maxLat, 
int=10, 
colbg="#FFFFFF",
colcon="#C8C8C8", 
colf="black", 
...
inc=0.005, 
save = "rds",
        file1 = "./output/comp_eco/Observed richness",
        file2 = "./output/comp_eco/List of species", 
        file3 = "./output/comp_eco/Species per site", 
        na= "NA", 
        dec = ",", 
        row.names = FALSE,
        jpg = TRUE, 
        jpg1 = "./output/comp_eco/Observed richness.jpg", 
        jpg2 = "./output/comp_eco/Records.jpg", 
        jpg3 = "./output/comp_eco/Completeness.jpg",
        jpg4 = "./output/comp_eco/Slope.jpg",
  ...
  load("output/completeness/Main30exp/Estimators.RData")
  Estimators <- values
  ...
  data(adworld)
    KnowB(data = result_df,
          ...
          Area = "World", 
          save = "RData",
          file4 = paste0("output/completeness/Main30exp/Estimators_", var))
  }
  
  # Load saved estimators for each family and store in the list
  for (var in families) {
  load(paste0("output/completeness/Main30exp/Estimators_", var, ".RData"))
  values$Family <- var
  ...
  save(EstimatorsAllFamilies, file="output/completeness/Main30exp/EstimatorsAllFamilies.RData")
  ...
  SurveyQ(values, Longitude=values$Longitude, Latitude=values$Latitude, cell=30, Areas=NULL,
          ...
          labels=TRUE, sizelabels=1, LEGENDP=NULL, LEGENDM=NULL, file="output/completeness/Main30exp/Polar coordinates.csv",
          na="NA", dec=",", row.names=FALSE, jpg=FALSE, filejpg="output/completeness/Main30exp/Map.jpg")
...
```{r}
     ...
     data="output/completeness/Main30exp/Estimators.RData"

     load(data)
     
     # Load country boundaries for mapping
     roads <- rnaturalearth::ne_countries(scale = 110,
                                          returnclass = "sf")
...
Repeating the steps of the chunk above.


For countries in the resolution 15 arc minutes.
```{r}
load("output/pams_for_completeness_v2.Rdata")
  ...
  KnowB(data = result_df,
      ...
      inc=0.005,
      save="RData",
        file1="output/completeness/Main15exp/Observed richness",
        file2="output/completeness/Main15exp/List of species", 
        file3="output/completeness/Main15exp/Species per site", 
        file4="output/completeness/Main15exp/Estimators",
        file5="output/completeness/Main15exp/Species per record", 
        file6="output/completeness/Main15exp/Records", 
        file7="output/completeness/Main15exp/Completeness", 
        file8="output/completeness/Main15exp/Slope",
        file9="output/completeness/Main15exp/Standard error of the estimators",
        na="NA", 
        dec=",", 
        row.names=FALSE,
        jpg=TRUE, 
        jpg1="output/completeness/Main15exp/Observed richness.jpg", 
        jpg2="output/completeness/Main15exp/Records.jpg", 
        jpg3="output/completeness/Main15exp/output/completeness.jpg",
        jpg4="output/completeness/Main15exp/Slope.jpg",
        cex=1.5, pch=15, cex.labels=1.5, pchcol="red", ask=FALSE)
  
  # list to store the results
  all_estimators <- list()
  
  load("output/completeness/Main15exp/Estimators.RData")
  Estimators <- values
  ...
  data(adworld)
    KnowB(data = result_df,
          ...
          Area="World", 
          save="RData",
          file4=paste0("output/completeness/Main15exp/Estimators_", var))
  }
  
  # Cargar archivos por familia fuera del loop
  for (var in families) {
  load(paste0("output/completeness/Main15exp/Estimators_", var, ".RData"))
  values$Family <- var
  all_estimators[[var]] <- values
  }
  ...
  # Bind rows and save
  EstimatorsAllFamilies <- bind_rows(all_estimators)
  save(EstimatorsAllFamilies, file="output/completeness/Main15exp/EstimatorsAllFamilies.RData")
  ...
  SurveyQ(values, Longitude=values$Longitude, Latitude=values$Latitude, cell=15, Areas=NULL,
          ...
          PLOTB=NULL, POINTS=NULL, XLAB=NULL, YLAB=NULL, XLIM=NULL, YLIM=NULL,
          palette=c("blue","green","red"), COLOR=c("red","green","blue"), colm="black",
          labels=TRUE, sizelabels=1, LEGENDP=NULL, LEGENDM=NULL, file="output/completeness/Main15exp/Polar coordinates.csv",
          na="NA", dec=",", row.names=FALSE, jpg=FALSE, filejpg="output/completeness/Main15exp/Map.jpg")
...
Process completeness for countries 15 arc minutes
```{r}

     data="output/completeness/Main15exp/Estimators.RData"
