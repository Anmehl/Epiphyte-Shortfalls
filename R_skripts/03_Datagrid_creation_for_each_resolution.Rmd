...
# Define file paths and projection settings
  ...
  data="data/occ_epIGbackbone.Rdata"# Input data file
...
# Map species occurrences to 1-degree grid
  ...
  gridded %>%  filter(!is.na(Accepted_Name)) %>%
    select(CellID_1d,Accepted_Name,family) %>%
  ...
  # Repeat the process for 0.5-degree, 0.25-degree, and 0.06-degree grids
 ...
 g <-  as(g, 'SpatialPixels')
  ...
  gridded %>% select(decimalLongitude,decimalLatitude) %>% 
    ...
    select(CellID_0.5d) %>% bind_cols(.,gridded) -> gridded
  ...
  gridded %>% filter(!is.na(Accepted_Name)) %>%
    select(CellID_0.5d,Accepted_Name,family) %>%
 ...
 g <-  as(g, 'SpatialPixels')
  ...
  gridded %>% select(decimalLongitude,decimalLatitude) %>% 
    ...
    select(CellID_0.2d) %>% bind_cols(.,gridded) -> gridded
  ...
  gridded %>% filter(!is.na(Accepted_Name)) %>%
    select(CellID_0.2d,Accepted_Name,family) %>% 
 ...
 g <-  as(g, 'SpatialPixels')
  ...
  gridded %>% select(decimalLongitude,decimalLatitude) %>% 
    ...
    sf::st_coordinates() %>% SpatialPoints(.,proj4string= CRS(projection)) %>%
    sp::over(.,g) %>% enframe(.,name="name") %>%
    rename_with(.,~all_of(c("name","CellID_0.1d"))) %>% select(CellID_0.1d) %>%
    bind_cols(.,gridded) -> gridded
  ...
  gridded %>% filter(!is.na(Accepted_Name)) %>%
    select(CellID_0.1d,Accepted_Name,family) %>% 
    group_by(CellID_0.1d,Accepted_Name,family) %>%
    summarize(Counts=n()) %>% ungroup() %>%
    right_join(.,coords,by="CellID_0.1d") -> tenth_degree
